...
const getDataInfo = async (val) => {
  if (val === "system") {
    console.log(route);
    const res = await http.getSystemInfo(route.query.ip);
    return res;
  } else if (val === "cpu") {
    const res = await http.getCpuInfo(route.query.ip);
    const obj = {};
    res.forEach((item) => {
      item.description
        .split(";")
    ...
    const res = await http.getBasicInfo(route.query.ip);
...
// const demo = () => {
//   tab.changeMenu('/pcinfo/file',{ a: 1 })
// };
// window.addEventListener("click", demo);
</script>
